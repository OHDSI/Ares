...
const getAnnotation = async (connection, annotationId) => {
  ...
  try {
    ...
    notes = notes.map((note) => ({
      ...note,
      lastUpdated: Number(note.lastUpdated.micros) / 1000,
      updatedAt: Number(note.updatedAt.micros) / 1000,
      createdAt: Number(note.createdAt.micros) / 1000,
    }));
  ...
  try {
    ...
    if (body) {
      ...
      if (notes) {
        ...
        let existingNotes = await connection.runAndReadAll(
