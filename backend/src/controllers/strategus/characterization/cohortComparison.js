...
export async function getCharacterizationCohortBinary({
  ...
  for (const r of merged) {
    ...
    row[`sumValue_${r.id}`] =
      r.averageValue < 0 ? -Math.abs(r.sumValue) : r.sumValue;
  ...
  for (const row of res) {
    for (const ref of colRef) {
      if (row[`averageValue_${ref.id}`] == null)
        row[`averageValue_${ref.id}`] = -1 * minThreshold;
      if (row[`sumValue_${ref.id}`] == null)
        row[`sumValue_${ref.id}`] = -1 * Math.floor(minThreshold * ref.n);
  ...
  if (colRef.length === 2 && sumCols.length === 2) {
    for (const row of res) {
      const sd1 =
        ((Math.abs(row.averageValue_1) - 1) ** 2 * Math.abs(row.sumValue_1) +
          (Math.abs(row.averageValue_1) - 0) ** 2 *
            (row.n_1 - Math.abs(row.sumValue_1))) /
        row.n_1;
      const sd2 =
        ((Math.abs(row.averageValue_2) - 1) ** 2 * Math.abs(row.sumValue_2) +
          (Math.abs(row.averageValue_2) - 0) ** 2 *
            (row.n_2 - Math.abs(row.sumValue_2))) /
        row.n_2;
      const denom = Math.sqrt((sd1 ** 2 + sd2 ** 2) / 2);
      row.SMD =
        denom === 0 ? 0 : (row.averageValue_1 - row.averageValue_2) / denom;
...
export async function getCharacterizationCohortContinuous({
  ...
  const colRef = counts.map((r, i) => ({
    ...r,
    id: i + 1,
    minCount: r.n * minThreshold,
  }));
  if (colRef.length === 0) return { covariates: [], covRef: [] };
  ...
  if (colRef.length === 2 && countCols.length === 2) {
    for (const row of res) {
      const denom = Math.sqrt(
        (Math.abs(row.standardDeviation_1) ** 2 +
          Math.abs(row.standardDeviation_2) ** 2) /
          2,
      );
      row.SMD =
        denom === 0
          ? 0
          : (Math.abs(row.averageValue_1) - Math.abs(row.averageValue_2)) /
            denom;
      row.absSMD = Math.abs(row.SMD);
    }
  }
