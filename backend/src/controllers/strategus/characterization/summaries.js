...
import {
  ...
  extractSubsetCohorts,
} from "./helpers/cohortDefinitions.js";
import {
  getIncidenceTargets,
  getIncidenceOutcomes,
} from "./helpers/incidenceTargets.js";
import {
  getCharacterizationTargets,
  getCharacterizationOutcomes,
} from "./helpers/characterizationTargets.js";
import {
  getPredictionTargets,
  getPredictionOutcomes,
} from "./helpers/predictionTargets.js";
...
function summarizeCounts(rawCounts) {
  ...
  for (const [cohortId, rows] of groups) {
    ...
    result.push({
      ...
      databaseStringCount: [
        ...new Set(rows.map((r) => `${r.databaseName} (${r.cohortSubjects})`)),
      ].join(", "),
...
export async function getTargetTable({
  ...
  if (printTimes)
    console.log(`extracting target cohorts: ${Date.now() - start}ms`);
  start = Date.now();
  ...
  if (printTimes)
    console.log(`processing target cohorts: ${Date.now() - start}ms`);
  start = Date.now();
  ...
  if (printTimes)
    console.log(`extracting target cohort counts: ${Date.now() - start}ms`);

  if (getIncidenceInclusion) {
    start = Date.now();
    const inc = await safeCall(() =>
      getIncidenceTargets({ schema, cgTablePrefix, ciTablePrefix }),
    );
    if (inc)
    ...
    if (printTimes)
      console.log(`extracting incidence targets: ${Date.now() - start}ms`);
  }
  ...
  if (getCharacterizationInclusion) {
    ...
    if (printTimes)
      console.log(
        `extracting characterization targets: ${Date.now() - start}ms`,
      );
  }
  ...
  if (getPredictionInclusion) {
    ...
    const pred = await safeCall(() =>
      getPredictionTargets({ schema, cgTablePrefix, plpTablePrefix }),
    );
    if (pred)
      cohortCounts = leftMerge(
        cohortCounts,
    ...
    if (printTimes)
      console.log(`extracting prediction targets: ${Date.now() - start}ms`);
  }

  if (getCohortMethodInclusion) {
    start = Date.now();
    const cm = await safeCall(() =>
      getCmTargets({ schema, cgTablePrefix, cmTablePrefix }),
    );
    if (cm)
      cohortCounts = leftMerge(
        cohortCounts,
    ...
    if (printTimes)
      console.log(`extracting cohort method targets: ${Date.now() - start}ms`);
  }

  if (getSccsInclusion) {
    start = Date.now();
    const sccs = await safeCall(() =>
      getSccsTargets({ schema, cgTablePrefix, sccsTablePrefix }),
    );
    if (sccs)
      cohortCounts = leftMerge(
        cohortCounts,
    ...
    if (printTimes)
      console.log(`extracting sccs targets: ${Date.now() - start}ms`);
  }
  ...
  console.log(
    `-- Total time for extracting target table: ${Date.now() - totalStart}ms`,
  );
  return cohortCounts;
}
...
export async function getOutcomeTable({
  ...
  const [rawCohorts, rawCounts, inc, char, pred, cm, sccs] = await Promise.all([
    ...
    getCohortMethodInclusion
      ? safeCall(() =>
          getCmOutcomes({ schema, cgTablePrefix, cmTablePrefix, targetId }),
        )
      : null,
    getSccsInclusion
      ? safeCall(() =>
          getSccsOutcomes({ schema, cgTablePrefix, sccsTablePrefix, targetId }),
        )
      : null,
  ]);
