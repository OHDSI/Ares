...
import { addOptionalClause } from "#shared/queryHelpers.js";
import type {
  DechallengeOptions,
...
export async function getDechallengeRechallenge({
  ...
  targetIds = null,
  outcomeIds = null,
}: DechallengeOptions): Promise<DeChallengeRechallengeResult[]> {
  ...
  return queryDb(sql, params) as unknown as Promise<
    DeChallengeRechallengeResult[]
  >;
}
...
export async function getDechallengeRechallengeFails({
...
}: DechallengeFailOptions): Promise<DeChallengeFailRow[]> {
  if (targetId === null || targetId === undefined)
    throw new Error("Must specify exactly one targetId");
  if (outcomeId === null || outcomeId === undefined)
    throw new Error("Must specify exactly one outcomeId");
  if (databaseId === null || databaseId === undefined)
    throw new Error("Must specify exactly one databaseId");

  const params: NamedParams = { targetId, outcomeId, databaseId };

  const stopClause = addOptionalClause(
    dechallengeStopInterval !== null && dechallengeStopInterval !== undefined,
    "AND DECHALLENGE_STOP_INTERVAL = @dechallengeStopInterval",
  );
  if (dechallengeStopInterval !== null && dechallengeStopInterval !== undefined)
    params["dechallengeStopInterval"] = dechallengeStopInterval;

  const evalClause = addOptionalClause(
    dechallengeEvaluationWindow !== null &&
      dechallengeEvaluationWindow !== undefined,
  ...
  return queryDb(sql, params) as Promise<DeChallengeFailRow[]>;
}
