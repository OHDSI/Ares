...
export function processBinaryRiskFactorFeatures({
  ...
  for (const cc of caseCounts) {
    ...
    for (const t of targetFeats) {
      ...
      const std1 =
        caseCount.personCount === 0
          ? 0
          : Math.sqrt(
              ((1 - ca) ** 2 * cc_val +
                (-ca) ** 2 * (caseCount.personCount - cc_val)) /
                caseCount.personCount,
            );
      const std2 =
        nonCaseCount === 0
          ? 0
  ...
  for (const o of outcomes) {
    ...
    for (const t of renamedTargets) {
      ...
      const denom = Math.sqrt(
        (caseStandardDeviation ** 2 + t.targetStandardDeviation ** 2) / 2,
      );
      const SMD =
        denom === 0 ? 0 : (caseAverageValue - t.targetAverageValue) / denom;
