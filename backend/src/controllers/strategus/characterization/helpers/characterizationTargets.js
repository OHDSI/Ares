...
export async function getCharacterizationTargets({
    ...
    if (useTte) {
        ...
        let tableOrView;
        try {
            await queryDb(`SELECT * FROM ${schema}.${cTablePrefix}time_to_event_targets LIMIT 1`);
    ...
    `);
        ...
        if (printTimes) console.log(`extracting risk factor data: ${Date.now() - start}ms`);
    ...
    `);
        ...
        if (printTimes) console.log(`extracting database comparator data: ${Date.now() - start}ms`);
    }

    if (allRows.length === 0) {
        console.log('No target data');
        console.log(`-- all extracting characterization targets took: ${Date.now() - totalStart}ms`);
        return null;
    }

    const colTypes = ['timeToEvent', 'dechalRechal', 'riskFactors', 'databaseComparator'];
...
export async function getCharacterizationOutcomes({
    ...
    const drTargetClause = addOptionalClause(targetId != null,
