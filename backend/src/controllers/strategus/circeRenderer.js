...
function codesetName(id, defaultName, conceptSets) {
  if (id == null || id === "") return defaultName;
...
function renderNumericRange(range) {
  if (!range) return "";
  const op = range.Op ?? range.op ?? "";
...
function renderTextFilter(filter) {
  if (!filter) return "";
...
const DATE_OFFSET_FIELD_OPTIONS = [
  { id: "StartDate", name: "start date" },
  { id: "EndDate", name: "end date" },
];

function renderDateAdjustment(da) {
...
function renderUserDefinedPeriod(p) {
  if (!p) return "";
...
// Window
function renderWindow(w, indexLabel = "cohort entry") {
  if (!w) return "";
  ...
  const dir = (coeff) => (coeff < 0 ? "before" : "after");
...
// criteriaTypes helpers

function renderAgeGender(c) {
  ...
  if (gender?.length || ageAtStart || ageAtEnd) {
    let desc = "who are";
    if (gender?.length) {
      desc += ` ${renderConceptList(gender, "")}`;
      if ((ageAtStart || ageAtEnd) && gender.length > 1) desc += ",";
    }
    if (ageAtStart) {
      desc += ` ${renderNumericRange(ageAtStart)} years old`;
      if (ageAtEnd) desc += " at era start and";
    }
    if (ageAtEnd) {
      desc += ` ${renderNumericRange(ageAtEnd)} years old at era end`;
    }
    if (genderCS) desc += ";";
    parts.push(desc);
  }
...
function renderEventDateCriteria(startRange, endRange) {
...
function renderWindowCriteria(countCriteria, indexLabel = "cohort entry") {
  ...
  if (sw) {
    ...
    if ((s.Days ?? s.days) != null || (e.Days ?? e.days) != null)
...
function renderCriteria(
  ...
  if (da) {
    const daStr = renderDateAdjustment(da);
  ...
  const csName = codesetName(codesetId, meta.default, conceptSets);
  ...
  const corr = c.CorrelatedCriteria;
  if (corr) {
...
function renderStrategy(endStrategy, conceptSets) {
...
export function renderCohortMarkdown(expressionJson) {
  ...
  const conceptSets = Array.isArray(rawCs)
  ...
  if (inclusionRules.length > 0) {
    ...
    inclusionRules.forEach((rule, i) => {
      ...
      lines.push(`\n#### ${i + 1}. ${name}${desc ? ": " + desc + "  " : ""}\n`);
      const expr = rule.expression ?? rule.Expression ?? {};
      lines.push(`\nEntry events ${renderGroup(expr, conceptSets, 0)}\n`);
    });
  }

  // Expression limit (no additional criteria)
